...
namespace PredatorControlApp
{
    ...
    public class WmiController : IDisposable
    {
        ...
        #region Hardware Features (GPU MUX, LCD Overdrive, Backlight Sleep, Boot Sound, CoolBoost)

        public async Task<bool> SetGpuModeAsync(int mode)
        {
            return await AcerAgentClient.SetGpuModeAsync(mode);
        }
        ...
        public async Task<bool> SetLcdOverdriveAsync(bool enable)
        {
            return await AcerAgentClient.SetLcdOverdriveAsync(enable);
        }

        public async Task<bool?> GetLcdOverdriveAsync()
        {
            return await AcerAgentClient.GetLcdOverdriveAsync();
        }

        public async Task<bool> SetBacklight30sAsync(bool enable)
        {
            // 1. Send via OEM Agent IPC
            bool agentOk = await AcerAgentClient.SetBacklight30sAsync(enable);

            // 2. Dual-dispatch: Send the exact 8-byte HID keyboard report
            // Format: [0x30, 0x01, status, 0, 0, 0, 0, 0xCE - status]
            byte status = (byte)(enable ? 1 : 0);
            byte[] kbPayload = new byte[8] { 0x30, 0x01, status, 0x00, 0x00, 0x00, 0x00, (byte)(0xCE - status) };
            try { SendGamingRgbKbCommand(kbPayload); } catch { }

            return agentOk;
        }

        public async Task<bool?> GetBacklight30sAsync()
        {
            return await AcerAgentClient.GetBacklight30sAsync();
        }

        public async Task<bool> SetBootSoundAsync(bool enable)
        {
            // 1. Send via OEM Agent IPC
            bool agentOk = await AcerAgentClient.SetBootSoundAsync(enable);

            // 2. Dual-dispatch: Send ACPI WMI SetGamingMiscSetting Register 4 (Boot Sound)
            try
            {
                SendCommand("SetGamingMiscSetting", 0x04UL | ((ulong)(enable ? 1 : 0) << 8));
            }
            catch { }
        ...
        public async Task<bool?> GetBootSoundAsync()
        {
            return await AcerAgentClient.GetBootSoundAsync();
        }

        public async Task<bool> SetBootAnimationAsync(bool enable)
        {
            // In PredatorSense, Boot Animation and Boot Sound are the single combined "BOOT_SOUND" setting
            return await SetBootSoundAsync(enable);
        }

        public async Task<bool?> GetBootAnimationAsync()
        {
            return await AcerAgentClient.GetBootAnimationAsync();
        }

        public async Task<bool> SetCoolBoostAsync(bool enable)
        {
            return await AcerAgentClient.SetCoolBoostAsync(enable);
        }

        public async Task<bool> SetWinKeyLockAsync(bool lockKeys)
        {
            // 1. Send via OEM Agent IPC
            bool agentOk = await AcerAgentClient.SetWinKeyLockAsync(lockKeys);

            // 2. Dual-dispatch: Send the exact 8-byte HID keyboard report
            // Format: [0x03, lockKeys ? 0 : 3, 0, 0, 0, 0, 0, 0xFC - (lockKeys ? 0 : 3)]
