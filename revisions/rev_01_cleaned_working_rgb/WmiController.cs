...
namespace PredatorControlApp
{
    ...
    public class WmiController : IDisposable
    {
        ...
        private void Apply4ZoneLightingCore(int mode)
        {
            ...
            // If all 4 zones are identical, send a single 0x0F mask command!
            if (_zoneColors[0] == _zoneColors[1] && _zoneColors[1] == _zoneColors[2] && _zoneColors[2] == _zoneColors[3])
            {
                Thread.Sleep(20);
                ApplyMultiZoneLightingCore(0x0Ful, _zoneColors[0]);
                return;
            }
            ...
            {
                Thread.Sleep(30); // Pacing for EC firmware buffer
