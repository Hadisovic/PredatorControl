...
export function App() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribe(m => {
      ...
      if (m.type === 'layout') setLayout(m.data as Layout)
  ...
  return <main className={`${layout.surface} ${layout.flyout ? 'flyout' : ''} ${suspended ? 'suspended' : ''}`} onPointerDown={e => { if (e.target === e.currentTarget) send('surface', 'compact') }}>
    ...
    {layout.surface !== 'compact' && <div className="glass" style={layout.flyout ? {left: layout.panelLeft, width: 'min(430px, 100vw)', top: 0, height: '100vh'} : undefined}>
      <header><div><span className="eyebrow">PREDATOR CONTROL</span><h1>{layout.surface === 'summary' ? 'At a glance' : layout.surface === 'menu' ? 'Quick controls' : 'Your calm control center'}</h1></div><button className="close" aria-label="Collapse to Jelli" onClick={() => send('surface', 'compact')}>×</button></header>
      ...
      {layout.surface === 'dashboard' && <><nav aria-label="Dashboard sections">{tabs.map(t => <button key={t} className={tab === t ? 'active' : ''} aria-current={tab === t ? 'page' : undefined} onClick={() => setTab(t)}>{t}</button>)}</nav><div className="dashboard-content">
