...
namespace PredatorControlApp
{
    ...
    public class GameSyncForm : Form
    {
        ...
        private PredatorDropDown _cboLcdOverdrive = null!, _cboWinKey = null!;
        ...
        private void BuildUI()
        {
            ...
            MakeLabelIn(_pnlEditor, "LCD OVERDRIVE", 0, ey, FontSection, HeaderColor);
            ey += 24;
            _cboLcdOverdrive = MakeComboIn(_pnlEditor, new[] { "Don't Change", "Enable (3ms Boost)", "Disable" }, 0, ey, editorW);
            ey += sectionGap;

            MakeLabelIn(_pnlEditor, "WINDOWS & MENU KEYS", 0, ey, FontSection, HeaderColor);
            ey += 24;
            _cboWinKey = MakeComboIn(_pnlEditor, new[] { "Don't Change", "Lock During Game", "Keep Unlocked" }, 0, ey, editorW);
            ey += sectionGap;
        ...
        private void LoadProfileToEditor(GameProfile p)
        {
            ...
            if (p.LcdOverdrive == -1) _cboLcdOverdrive.SelectedIndex = 0;
            else if (p.LcdOverdrive == 1) _cboLcdOverdrive.SelectedIndex = 1;
            else _cboLcdOverdrive.SelectedIndex = 2;

            if (p.LockWinKey == -1) _cboWinKey.SelectedIndex = 0;
            else if (p.LockWinKey == 1) _cboWinKey.SelectedIndex = 1;
            else _cboWinKey.SelectedIndex = 2;
        ...
        private GameProfile EditorToProfile()
        {
            ...
            p.LcdOverdrive = _cboLcdOverdrive.SelectedIndex switch
            {
                1 => 1,
                2 => 0,
                _ => -1
            ...
            p.LockWinKey = _cboWinKey.SelectedIndex switch
            {
                1 => 1,
                2 => 0,
                _ => -1
