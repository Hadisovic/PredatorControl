...
namespace PredatorControlApp
{
    ...
    public partial class Form1 : Form
    {
        ...
        private PredatorToggle _switchCoolBoost = null!;
        private Label _lblCoolBoost = null!;
        ...
        private bool _isInitializingGpuMode = true;

        private PredatorToggle _switchWinKeyLock = null!;
        private Label _lblWinKeyLock = null!;

        private PredatorToggle _switchBootSound = null!;
        private Label _lblBootSound = null!;
        ...
        private void BuildTrayMenu()
        {
            ...
            var trayCoolBoost = new ToolStripMenuItem("CoolBoost™", null, (s, e) =>
            {
                if (_switchCoolBoost != null) _switchCoolBoost.Checked = !_switchCoolBoost.Checked;
            });
            ...
            hardwareMenu.DropDownItems.AddRange([trayWinKey, trayCoolBoost, trayLcdOverdrive]);
        ...
        private void BuildUI()
        {
            ...
            y += btnH + S(12);
            int switchH = S(30);
            _lblCoolBoost = MakeLabel("CoolBoost™ (Max Fan Boost)", pad, y, FontBody, Color.FromArgb(120, 120, 135));
            CenterV(_lblCoolBoost, y, switchH);
            ...
            _switchCoolBoost.CheckedChanged += (s, e) =>
            {
                ...
                SaveState("CoolBoost", enable ? 1 : 0);
                Task.Run(async () =>
                {
                    try { await _wmi.SetCoolBoostAsync(enable); } catch { }
                });
            };
            ...
            _switchLcdOverdrive.CheckedChanged += (s, e) =>
            {
                ...
                Task.Run(async () =>
                {
                    try { await _wmi.SetLcdOverdriveAsync(enable); } catch { }
                });
            };
            ...
            _switchBacklight30s.CheckedChanged += (s, e) =>
            {
                ...
                Task.Run(async () =>
                {
                    try { await _wmi.SetBacklight30sAsync(enable); } catch { }
                });
            };
            ...
            y += S(20);
            _cboGpuMode = new PredatorDropDown { Location = new Point(pad, y), Size = new Size(contentW, S(34)) };
            _cboGpuMode.Items.AddRange(["Optimus (Hybrid / Dynamic)", "NVIDIA GPU Only (Discrete)", "Auto (Advanced Optimus)"]);
            _cboGpuMode.SelectedIndex = 0;
            _contentPanel.Controls.Add(_cboGpuMode);

            _cboGpuMode.SelectedIndexChanged += (s, e) =>
            {
                if (_isInitializingGpuMode) return;

                int selectedIdx = _cboGpuMode.SelectedIndex;
                int targetMode = selectedIdx switch
                {
                    0 => 0, // Optimus (Hybrid / Dynamic)
                    1 => 1, // Discrete (NVIDIA GPU Only)
                    2 => 2, // Auto (Advanced Optimus)
                    _ => 0  // Default to Optimus
                };
                string modeName = selectedIdx switch
                {
                    1 => "NVIDIA GPU Only (Discrete)",
                    2 => "Auto (Advanced Optimus)",
                    _ => "Optimus (Dynamic / Hybrid)"
                };

                SaveState("GpuMode", targetMode);
                Task.Run(async () =>
                {
                    try { await _wmi.SetGpuModeAsync(targetMode); } catch { }
                });

                var dlgResult = MessageBox.Show(this,
                    $"GPU working mode changed to \"{modeName}\".\n\n" +
                    "A system restart is required for the hardware GPU MUX switch to take effect.\n\n" +
                    "Would you like to restart your computer now?",
                    "Predator Control - GPU MUX Restart Required",
                ...
                if (dlgResult == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show(this, $"Failed to initiate system restart:\n{ex.Message}", "Restart Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            };
            ...
            _switchWinKeyLock.CheckedChanged += (s, e) =>
            {
                ...
                Task.Run(async () =>
                {
                    try { await _wmi.SetWinKeyLockAsync(lockKeys); } catch { }
                });
            };

            // BIOS Boot Animation & Sound
            y += switchH + S(12);
            _lblBootSound = MakeLabel("BIOS Boot Animation & Sound", pad, y, FontBody, Color.FromArgb(120, 120, 135));
            ...
            _switchBootSound.CheckedChanged += (s, e) =>
            {
                ...
                SaveState("BootAnimation", enable ? 1 : 0);
                Task.Run(async () =>
                {
                    try
                    {
                        await _wmi.SetBootSoundAsync(enable);
                        await _wmi.SetBootAnimationAsync(enable);
                    }
                    catch { }
                });
            };
        ...
        private void ApplyPresetColor(Color c)
        {
            ...
            // Automatically switch to Static if currently in an animated mode
            if (_rgbDropDown.SelectedIndex != 0 && _rgbDropDown.SelectedIndex != 1)
            {
                _rgbDropDown.SelectedIndex = 0;
            }

            int mode = _rgbDropDown.SelectedIndex == 8 ? 0 : _rgbDropDown.SelectedIndex;
            ...
            else if (_selectedZone >= 0 && _selectedZone < 4)
            {
                ...
                _wmi.SetSingleZoneColor(_selectedZone, c, mode);
            }
        ...
        {
            // Only Static (0) and Breathing (1) support custom color customization.
            // Animated modes (Neon, Wave, Shifting, Zoom, Meteor, Twinkling, Off) control colors firmware-wide.
            bool colorsEnabled = (mode == 0 || mode == 1);

            if (_btnAllZones != null) _btnAllZones.Enabled = colorsEnabled;
            for (int z = 0; z < 4; z++)
            {
                if (_btnZones[z] != null) _btnZones[z].Enabled = colorsEnabled;
            }
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            if (_contentPanel != null && _lblTitle != null)
            {
                ...
                // CoolBoost, LCD Overdrive & Backlight 30s Switches
                ...
                // System & Hardware Controls
                if (_cboGpuMode != null) _cboGpuMode.Width = contentW;
        ...
        private async Task ApplyGameProfile(GameProfile profile)
        {
            ...
            if (profile.LcdOverdrive >= 0)
            {
                ...
                try { await _wmi.SetLcdOverdriveAsync(enableLcd); } catch { }
            }
        ...
        private async Task RestoreSnapshot(DashboardSnapshot snap)
        {
            ...
            if (_switchWinKeyLock != null)
            {
                ...
                try { await _wmi.SetWinKeyLockAsync(snap.WasWinKeyLocked); } catch { }
            }

            if (_switchLcdOverdrive != null)
            {
                _switchLcdOverdrive.Checked = snap.WasLcdOverdriveEnabled;
                try { await _wmi.SetLcdOverdriveAsync(snap.WasLcdOverdriveEnabled); } catch { }
            }
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                // CoolBoost
                int savedCoolBoost = GetInt(key, "CoolBoost", 0, 0, 1);
                if (_switchCoolBoost != null) _switchCoolBoost.Checked = (savedCoolBoost == 1);
                Task.Run(async () => { try { await _wmi.SetCoolBoostAsync(savedCoolBoost == 1); } catch { } });
                ...
                Task.Run(async () => { try { await _wmi.SetLcdOverdriveAsync(savedLcdOverdrive == 1); } catch { } });
                ...
                Task.Run(async () => { try { await _wmi.SetBacklight30sAsync(savedBacklight30s == 1); } catch { } });
                ...
                Task.Run(async () => { try { await _wmi.SetWinKeyLockAsync(savedWinKeyLock == 1); } catch { } });

                // BIOS Boot Animation & Sound (merged single toggle)
                int savedBootSound = GetInt(key, "BootSound", 1, 0, 1);
                if (_switchBootSound != null) _switchBootSound.Checked = (savedBootSound == 1);
                Task.Run(async () =>
                {
                    try
                    {
                        await _wmi.SetBootSoundAsync(savedBootSound == 1);
                        await _wmi.SetBootAnimationAsync(savedBootSound == 1);
                    }
                    catch { }
                });
                ...
                // GPU MUX Mode
                _isInitializingGpuMode = true;
                try
                {
                    int savedGpuMode = GetInt(key, "GpuMode", 0, 0, 2);
                    if (_cboGpuMode != null)
                    {
                        _cboGpuMode.SelectedIndex = savedGpuMode switch
                        {
                            1 => 1, // Discrete
                            2 => 2, // Auto
                            _ => 0  // Optimus
                    ...
                    Task.Run(async () => { try { await _wmi.SetGpuModeAsync(savedGpuMode); } catch { } });
                }
                finally
                {
                    _isInitializingGpuMode = false;
                }

                // Background query from AcerAgentService to synchronize actual hardware state
                Task.Run(async () =>
                {
                    try
                    {
                        var hwGpuMode = await _wmi.GetGpuModeAsync();
                        if (hwGpuMode.HasValue && !IsDisposed && IsHandleCreated)
                        {
                            BeginInvoke(new Action(() =>
                            {
                                int cboIdx = hwGpuMode.Value switch
                                {
                                    1 => 1,
                                    2 => 2,
                                    _ => 0
                                };
                                if (_cboGpuMode != null && _cboGpuMode.SelectedIndex != cboIdx)
                                {
                                    _isInitializingGpuMode = true;
                                    try { _cboGpuMode.SelectedIndex = cboIdx; }
                                    finally { _isInitializingGpuMode = false; }
                                }
                            }));
                        }

                        var hwOverdrive = await _wmi.GetLcdOverdriveAsync();
                        if (hwOverdrive.HasValue && !IsDisposed && IsHandleCreated)
                        {
                            BeginInvoke(new Action(() =>
                            {
                                if (_switchLcdOverdrive != null && _switchLcdOverdrive.Checked != hwOverdrive.Value)
                                    _switchLcdOverdrive.Checked = hwOverdrive.Value;
                            }));
                        }

                        var hwBacklight = await _wmi.GetBacklight30sAsync();
                        if (hwBacklight.HasValue && !IsDisposed && IsHandleCreated)
                        {
                            BeginInvoke(new Action(() =>
                            {
                                if (_switchBacklight30s != null && _switchBacklight30s.Checked != hwBacklight.Value)
                                    _switchBacklight30s.Checked = hwBacklight.Value;
                            }));
                        }

                        var hwBootSound = await _wmi.GetBootSoundAsync();
                        if (hwBootSound.HasValue && !IsDisposed && IsHandleCreated)
                        {
                            BeginInvoke(new Action(() =>
                            {
                                if (_switchBootSound != null && _switchBootSound.Checked != hwBootSound.Value)
                                    _switchBootSound.Checked = hwBootSound.Value;
                            }));
                        }
                    }
                    catch { }
