...
namespace PredatorControlApp
{
    ...
    public class WmiController : IDisposable
    {
        ...
        {
            // Step 1: Mandatory EC firmware LED behavior reset
            SendCommand("SetGamingLEDBehavior", 0x07ul);
            Thread.Sleep(50); // EC hardware requires 50ms to reset registers

            // Step 2: Zone color payloads (modes 2 Neon and 3 Wave handle colors in firmware animation)
            if (mode != 2 && mode != 3)
            {
            ...
            // Step 3: Commit backlight mode and brightness
            byte[] payload = new byte[16];
            payload[0] = (byte)mode;     // 0=Static, 1=Breathing, 2=Neon, 3=Wave, 4=Shifting, 5=Zoom, 6=Meteor, 7=Twinkling
            payload[1] = _speed;
