...
export const metrics: Record<string, {label: string; unit: string}> = {
  ...
  gpuPowerW: {label: 'GPU power', unit: 'W'}, cpuPowerW: {label: 'CPU power', unit: 'W'},
  ramUsedGb: {label: 'RAM used', unit: 'GB'}, vramUsedGb: {label: 'VRAM used', unit: 'GB'},
  batteryPercent: {label: 'Battery', unit: '%'}, powerMode: {label: 'Mode', unit: ''}, fps: {label: 'FPS', unit: ''},
}
export function valueFor(s: State | null, key: string) {
  ...
  if (key === 'ramUsedGb' || key === 'vramUsedGb' || key === 'gpuPowerW' || key === 'cpuPowerW') {
