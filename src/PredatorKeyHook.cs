...
namespace PredatorControlApp
{
    ...
    public sealed class PredatorKeyHook : IDisposable
    {
        ...
        /// <summary>
        /// In-Game Windows Key &amp; Menu Key Lock state.
        /// When true, suppresses VK_LWIN (0x5B), VK_RWIN (0x5C), and VK_APPS (0x5D).
        /// </summary>
        public static volatile bool IsWinKeyLocked = false;

        public static void SetWinKeyLocked(bool locked)
        {
            IsWinKeyLocked = locked;
            Task.Run(async () =>
            {
                try { await AcerAgentClient.SetWinKeyLockAsync(locked); } catch { }
            });
        }
        ...
        private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0 && (wParam == (IntPtr)WM_KEYDOWN || wParam == (IntPtr)WM_SYSKEYDOWN || wParam == (IntPtr)WM_KEYUP || wParam == (IntPtr)WM_SYSKEYUP))
            {
                ...
                if (IsWinKeyLocked && (kb.vkCode == 0x5B || kb.vkCode == 0x5C || kb.vkCode == 0x5D || kb.scanCode == 0x5B))
                {
                    return (IntPtr)1; // Suppress Windows key and context menu key
                }
